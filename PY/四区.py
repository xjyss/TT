...
class Spider(Spider):
    ...
    def _extractVideoItems(self, html_content):
        ...
        try:
            # 兼容多种列表模板
            items = re.findall(r'<div[^>]*class="xowe-thumb-bl"[^"]*"*href="([^"]+)"[^>]*>.*?<img[^>]*data-original="([^"]+)"[^>]*>.*?<div[^>]*class="xowe-thumb-name[^"]*"[^>]*>(.*?)</div>', html_content, re.S)
            if not items:
    ...
    def homeContent(self, filter):
        result = {}
        classes = [
            {'type_id': '/index.php/art/type/id/17.html', 'type_name': '中文字幕'},
            {'type_id': '/index.php/art/type/id/18.html', 'type_name': '无码破解'}
        ]
    ...
    def searchContent(self, key, quick, pg="1"):
        result = {'list': []}
        try:
            search_url = f"{self.host}/index.php/vod/type/id/1/wd/{requests.utils.quote(key)}/page/{pg}.html"
