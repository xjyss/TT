...
class Spider(BaseSpider):
    ...
    def get_working_host(self):
        dynamic_urls = [
            'https://attempt.civktfijf.com',
            'https://bible.civktfijf.com',
            'https://bottle.civktfijf.com'
           #91视频发布页 'https://91spx21.com'
        ]
    ...
    def homeContent(self, filter):
            ...
            classes = [
                 {'type_name': '国产自拍','type_id': '/videos/category/gczp/'},
                 {'type_name': '国产传媒','type_id': '/videos/category/gccm/'},
                 {'type_name': '经典三级','type_id': '/videos/category/gtsj/'},
                 {'type_name': '捆绑调教','type_id': '/videos/category/kbtj/'},
                 {'type_name': '暗网猎奇','type_id': '/videos/category/awlq/'},
                 {'type_name': '真实迷玩','type_id': '/videos/category/zsmw/'},
                 {'type_name': '醉酒剧情','type_id': '/videos/category/zjjq/'},
                 {'type_name': '按摩探花','type_id': '/videos/category/thjx/'},
                 {'type_name': '酒店监控','type_id': '/videos/category/jdjk/'},
                 {'type_name': '真实厕拍','type_id': '/videos/category/zscp/'},
                 {'type_name': '街头抄底','type_id': '/videos/category/jtcd/'},
                 {'type_name': 'AI换脸','type_id': '/videos/category/aihl/'},
                 {'type_name': 'AI漫剧','type_id': '/videos/category/aimj/'},
                 {'type_name': 'AI成人短剧','type_id': '/videos/category/aicrdj/'},
                 {'type_name': '日本AV','type_id': '/videos/category/rbav/'},
                 {'type_name': '疯狂老外','type_id': '/videos/category/fklw/'},
                 {'type_name': '91原创','type_id': '/videos/category/91yc/'},
                 {'type_name': '海角乱伦','type_id': '/videos/category/hjll/'},
                 {'type_name': '网红黑料','type_id': '/videos/category/whhl/'},
                 {'type_name': '反差婊','type_id': '/videos/category/fcb/'},
                 {'type_name': '街头搭讪','type_id': '/videos/category/gyds/'},
                 {'type_name': 'H动漫','type_id': '/videos/category/hdm/'},
                 {'type_name': '热门搜索','type_id': '/videos/hot/list/'},
                 {'type_name': '回家方法','type_id': '/home/contact/'}
          ]
    ...
    def categoryContent(self, tid, pg, filter, extend):
        try:
            ...
            response = self.session.get(url, proxies=self.proxies, timeout=7)
            if response.status_code != 200: return {'list': [], 'page': pg, 'pagecount': 30, 'limit': 90, 'total': 0}
    ...
    def detailContent(self, ids):
        try:
            ...
            response = self.session.get(url, headers=self.headers, proxies=self.proxies, timeout=7)
            data = self.getpq(response.text)
                    ...
                    try:
                        ...
                        if config_attr:
                            ...
                            if video_url:
                                ...
                                for _ in range(0):
                                    if not parent: break
                                    heading = parent.find('h1, h2, h3, h4').eq(0).text().strip()
                                    if heading:
                                        ep_name = heading
                                        break
            ...
            if not plist:
                content_area = data('.post-content, li')
            ...
            play_url = '#'.join(plist) if plist else f"正片${url}"
            ...
            try:
                ...
                if tags:
                    ...
                    vod_content = data('.post-card-title').text()
            except Exception:
                vod_content = '获取标签失败'

            if not vod_content:
                vod_content = data('h2').text() or '91视频'

            return {'list': [{'vod_play_from': '91视频', 'vod_play_url': play_url, 'vod_content': vod_content}]}
        except:
            return {'list': [{'vod_play_from': '91视频', 'vod_play_url': '获取失败'}]}
    ...
    def searchContent(self, key, quick, pg="1"):
        try:
            ...
            if pg == 1:
                url = f"{self.host}/videos/tag/{key}/"
            else:
                url = f"{self.host}/videos/tag/{key}/{pg}/"
            
            response = self.session.get(url, headers=self.headers, proxies=self.proxies, timeout=15)
            return {'list': self.getlist(self.getpq(response.text)('li')), 'page': pg, 'pagecount': 9999}
        except:
    ...
    def getfod(self, id):
        url = f"{self.host}{id}"
        data = self.getpq(self.session.get(url, headers=self.headers, proxies=self.proxies).text)
        videos = []
    ...
    def getimg(self, text, elem=None, html_content=None):
        ...
        if 'url(' in html_content:
            m = re.search(r'data-src=*\(\s*[\'"]?([^"\'\)]+)[\'"]?\s*', html_content, re.I)
            if m: return self._proc_url(m.group(1))
