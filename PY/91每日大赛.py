...
class Spider(BaseSpider):
    ...
    def homeContent(self, filter):
            ...
            classes = [
                 {'type_name': '今日爆料','type_id': '/category/14'},
                 {'type_name': '必撸精品','type_id': '/category/bljp/'},
                 {'type_name': '热搜爆料','type_id': '/category/rmdg/'},
                 {'type_name': 'AI短剧','type_id': '/category/rmdj/'},
                 {'type_name': '学生校园','type_id': '/category/xsxy/'},
                 {'type_name': '海角乱伦','type_id': '/category/hjll/'},
                 {'type_name': '每日大赛','type_id': '/category/mrds/'},
                 {'type_name': '探花精选','type_id': '/category/thjp/'},
                 {'type_name': '网红明星','type_id': '/category/mxhl/'},
                 {'type_name': '反差吃瓜','type_id': '/category/fccg/'},
                 {'type_name': '吃瓜猎奇','type_id': '/category/cgxw/'},
                 {'type_name': '成人动漫','type_id': '/category/dmwm/'},
                 {'type_name': '混剪PMV','type_id': '/category/dyfjx/'},
                 {'type_name': '寸止挑战','type_id': '/category/cztz/'},
                 {'type_name': '无码AV','type_id': '/category/wmav/'},
                 {'type_name': '欧美精品','type_id': '/category/omjp/'},
                 {'type_name': '吃瓜榜单','type_id': '/category/bkdg/'},
                 {'type_name': '美加墨世界杯','type_id': '/category/sjb/'}
          ]
    ...
    def categoryContent(self, tid, pg, filter, extend):
        try:
            ...
            videos = self.getlist(data('a'), tid)
    ...
    def detailContent(self, ids):
        try:
                    ...
                    try:
                        ...
                        if config_attr:
                            ...
                            if video_url:
                                ...
                                for _ in range(2):
                                    if not parent: break
                                    heading = parent.find('h1,h2, h3, h4').eq(1).text().strip()
                                    if heading:
                                        ep_name = heading
                                        break
            ...
            if not vod_content:
                vod_content = data('h2').text() or '91每日大赛'

            return {'list': [{'vod_play_from': '91每日大赛', 'vod_play_url': play_url, 'vod_content': vod_content}]}
        except:
            return {'list': [{'vod_play_from': '91每日大赛', 'vod_play_url': '获取失败'}]}
    ...
    def getlist(self, data, tid=''):
        ...
        for k in data.items():
            ...
            href = a.attr('href')
            title = k('h3').text() or k('.list-h-card__title').text() or k('.post-title').text()
            ...
            if href and title:
                ...
                videos.append({
                    ...
                    'vod_pic': img,
                    'vod_remarks': (k('.list-h-card__meta>span').eq(3).text() or '').strip('•'),
    ...
    def getfod(self, id):
        ...
        videos = []
        for i, h3 in enumerate(data('.list-h-card__title h3').items()):
